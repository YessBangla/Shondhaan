...
import {
  Search, MapPin, Filter, Eye, Clock, Star, Grid, List, SlidersHorizontal,
  Home as HomeIcon, ChevronRight, Heart, X, Image as ImageIcon, Phone, Tag,
  ChevronDown, BadgeCheck, Truck, RefreshCw, Building2,
} from "lucide-react";
...
const DealAllAds = () => {
  ...
  // Extra bikroy-style filters (client-side)
  const [selectedCats, setSelectedCats] = useState<string[]>([]); // category slugs
  ...
  const [postedWithin, setPostedWithin] = useState<string>("any"); // any | 24h | 7d | 30d
  ...
  const { data: listings, isLoading } = useDealListings({
    ...
    sortBy: sortBy === "price_asc" || sortBy === "price_desc" ? sortBy : undefined,
  });

  // Client-side filters layered on top of server filters
  const allCats = categoryTree?.flatMap((c) => [c, ...(c.children || [])]) || [];
  const slugToId = new Map(allCats.map((c) => [c.slug, c.id]));
  const selectedCatIds = selectedCats.map((s) => slugToId.get(s)).filter(Boolean) as string[];

  let visibleListings = listings ? [...listings] : [];
  if (promotedOnly) visibleListings = visibleListings.filter((l) => l.is_featured);
  if (selectedCatIds.length > 0) visibleListings = visibleListings.filter((l) => l.category_id && selectedCatIds.includes(l.category_id));
  if (withImageOnly) visibleListings = visibleListings.filter((l) => (l.images?.length || 0) > 0);
  if (withPhoneOnly) visibleListings = visibleListings.filter((l) => l.phone && !l.hide_phone);
  if (negotiableOnly) visibleListings = visibleListings.filter((l) => l.is_negotiable);

  // Posted-within filter (client-side)
  if (postedWithin !== "any") {
    const now = Date.now();
    const ms = postedWithin === "24h" ? 86400000 : postedWithin === "7d" ? 7 * 86400000 : 30 * 86400000;
    visibleListings = visibleListings.filter((l) => now - new Date(l.created_at).getTime() <= ms);
  }

  // Extra sort options (client-side)
  if (sortBy === "date_asc") {
    visibleListings.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
  } else if (sortBy === "popular") {
    visibleListings.sort((a, b) => (b.views_count || 0) - (a.views_count || 0));
  }

  const totalCount = visibleListings.length;
  const totalPages = Math.max(1, Math.ceil(totalCount / PAGE_SIZE));
  const currentPage = Math.min(page, totalPages);
  const pagedListings = visibleListings.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);

  // Reset page when filters change
  useEffect(() => { setPage(1); }, [search, locationFilter, condition, priceRange, sortBy, selectedCats.join(","), withImageOnly, withPhoneOnly, negotiableOnly, promotedOnly]);
  ...
  // Bikroy-style hierarchical category tree (parent -> children, expand/collapse)
  const CategoryTree = () => (
    <div className="space-y-0.5 max-h-[480px] overflow-y-auto pr-1 -mx-1">
      {categoryTree?.map((cat) => {
        ...
        return (
          <div key={cat.id}>
            <div className={`flex items-center gap-1.5 px-1.5 py-1.5 rounded-md hover:bg-muted/70 transition-colors ${checked ? "bg-primary/10" : ""}`}>
              {hasChildren ? (
                <button
                  type="button"
              ...
              >
                ...
                <span className={`flex-1 truncate ${checked ? "font-semibold text-primary" : "text-foreground"}`}>
            ...
            {hasChildren && isOpen && (
              <div className="ml-7 border-l border-border/60 pl-2 mt-0.5 mb-1 space-y-0.5">
                {cat.children!.map((child) => {
                  const cChecked = selectedCats.includes(child.slug);
                  return (
                    <button
                      type="button"
                      key={child.id}
                      onClick={() => toggleCat(child.slug)}
                      className={`w-full flex items-center gap-2 px-1.5 py-1 rounded-md text-left text-[13px] hover:bg-muted/70 transition-colors ${cChecked ? "bg-primary/10 font-semibold text-primary" : "text-foreground"}`}
                    >
                      <span className="shrink-0">{child.icon || "•"}</span>
                      <span className="truncate flex-1">{bn ? child.name : (child.name_en || child.name)}</span>
                    </button>
                  );
                })}
              </div>
  ...
  const FilterPanel = () => (
    <div className="space-y-5">
      ...
      <div>
        <label className="text-sm font-medium text-foreground mb-2 block">{bn ? "মূল্য (৳)" : "Price (৳)"}</label>
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          {[
            { l: "<5K", v: [0, 5000] as [number, number] },
            { l: "5K-25K", v: [5000, 25000] as [number, number] },
            { l: "25K-1L", v: [25000, 100000] as [number, number] },
            { l: "1L-5L", v: [100000, 500000] as [number, number] },
            { l: "5L+", v: [500000, 10000000] as [number, number] },
          ].map((p) => (
            <button
              key={p.l}
              type="button"
      ...
      <div>
        <label className="text-sm font-medium text-foreground mb-2 block">{bn ? "পোস্ট করা হয়েছে" : "Posted within"}</label>
        <Select value={postedWithin} onValueChange={setPostedWithin}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="any">{bn ? "যেকোনো সময়" : "Any time"}</SelectItem>
            <SelectItem value="24h">{bn ? "গত ২৪ ঘণ্টা" : "Last 24 hours"}</SelectItem>
            <SelectItem value="7d">{bn ? "গত ৭ দিন" : "Last 7 days"}</SelectItem>
            <SelectItem value="30d">{bn ? "গত ৩০ দিন" : "Last 30 days"}</SelectItem>
          </SelectContent>
        </Select>
      </div>

      <div className="space-y-2 pt-1">
        <label className="text-sm font-medium text-foreground mb-1 block">{bn ? "অতিরিক্ত ফিল্টার" : "More Filters"}</label>
        <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
          <Checkbox checked={withImageOnly} onCheckedChange={(v) => setWithImageOnly(!!v)} />
          <ImageIcon className="h-3.5 w-3.5 text-muted-foreground" />
          <span>{bn ? "শুধু ছবি সহ" : "With image only"}</span>
        </label>
        <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
          <Checkbox checked={withPhoneOnly} onCheckedChange={(v) => setWithPhoneOnly(!!v)} />
          <Phone className="h-3.5 w-3.5 text-muted-foreground" />
          <span>{bn ? "ফোন নম্বর সহ" : "With phone number"}</span>
        </label>
        <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
          <Checkbox checked={negotiableOnly} onCheckedChange={(v) => setNegotiableOnly(!!v)} />
          <Tag className="h-3.5 w-3.5 text-muted-foreground" />
          <span>{bn ? "দরদাম যোগ্য" : "Negotiable"}</span>
        </label>
        <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
          <Checkbox checked={promotedOnly} onCheckedChange={(v) => setPromotedOnly(!!v)} />
      ...
      <div className="space-y-2 pt-1">
        <label className="text-sm font-medium text-foreground mb-1 block">{bn ? "বিক্রেতার ধরন" : "Seller Type"}</label>
        <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
      ...
      {activeFilterCount > 0 && (
        <Button variant="outline" size="sm" onClick={clearAllFilters} className="w-full gap-2">
          ...
          {bn ? `সকল ফিল্টার মুছুন (${activeFilterCount})` : `Clear all filters (${activeFilterCount})`}
        </Button>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      {/* Breadcrumb + Page Title (bikroy style) */}
      <div className="bg-background border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <nav className="flex items-center gap-1.5 text-xs text-muted-foreground mb-2">
            <button onClick={() => navigate("/")} className="hover:text-primary flex items-center gap-1">
              <HomeIcon className="h-3 w-3" /> {bn ? "হোম" : "Home"}
            </button>
            <ChevronRight className="h-3 w-3" />
            <button onClick={() => navigate("/deal")} className="hover:text-primary">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {visibleListings?.length || 0} {bn ? "টি বিজ্ঞাপন পাওয়া গেছে" : " ads found"}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 pb-28 md:pb-10">
        ...
        <div className="flex gap-2 mb-4">
          <div className="relative flex-1">
            ...
            <Input
              placeholder={bn ? "বাংলাদেশের সকল বিজ্ঞাপনে খুঁজুন..." : "Search all ads in Bangladesh..."}
              value={search}
          ...
          <Select value={sortBy} onValueChange={setSortBy}>
            ...
            <SelectContent>
              <SelectItem value="newest">{bn ? "তারিখ: নতুন আগে" : "Date: Newest"}</SelectItem>
              <SelectItem value="date_asc">{bn ? "তারিখ: পুরাতন আগে" : "Date: Oldest"}</SelectItem>
              <SelectItem value="price_asc">{bn ? "কম মূল্য" : "Price ↑"}</SelectItem>
              <SelectItem value="price_desc">{bn ? "বেশি মূল্য" : "Price ↓"}</SelectItem>
              <SelectItem value="popular">{bn ? "জনপ্রিয়" : "Most Popular"}</SelectItem>
            </SelectContent>
          </Select>
        ...
        {activeFilterCount > 0 && (
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className="text-xs font-medium text-muted-foreground">{bn ? "সক্রিয় ফিল্টার:" : "Active filters:"}</span>
            {locationFilter.division && (
              <Badge variant="secondary" className="gap-1">
                <MapPin className="h-3 w-3" /> {locationFilter.division}
                <button onClick={() => setLocationFilter({ division: "", district: "", thana: "" })}><X className="h-3 w-3" /></button>
              </Badge>
            )}
            {condition !== "all" && (
              <Badge variant="secondary" className="gap-1">
            ...
            {(priceRange[0] > 0 || priceRange[1] < 10000000) && (
              <Badge variant="secondary" className="gap-1">
                ৳{priceRange[0].toLocaleString("bn-BD")} – ৳{priceRange[1].toLocaleString("bn-BD")}
                <button onClick={() => setPriceRange([0, 10000000])}><X className="h-3 w-3" /></button>
              </Badge>
            )}
            {selectedCats.map((slug) => {
              ...
              return (
                <Badge key={slug} variant="secondary" className="gap-1">
                  ...
                  <button onClick={() => toggleCat(slug)}><X className="h-3 w-3" /></button>
                </Badge>
              );
            })}
            {withImageOnly && <Badge variant="secondary" className="gap-1">{bn ? "ছবি সহ" : "With image"}<button onClick={() => setWithImageOnly(false)}><X className="h-3 w-3" /></button></Badge>}
            {withPhoneOnly && <Badge variant="secondary" className="gap-1">{bn ? "ফোন সহ" : "With phone"}<button onClick={() => setWithPhoneOnly(false)}><X className="h-3 w-3" /></button></Badge>}
            {negotiableOnly && <Badge variant="secondary" className="gap-1">{bn ? "দরদাম" : "Negotiable"}<button onClick={() => setNegotiableOnly(false)}><X className="h-3 w-3" /></button></Badge>}
            {promotedOnly && <Badge variant="secondary" className="gap-1">{bn ? "প্রমোটেড" : "Promoted"}<button onClick={() => setPromotedOnly(false)}><X className="h-3 w-3" /></button></Badge>}
        ...
        <div className="flex gap-5">
          {/* Sidebar - bikroy style: categories + filters */}
            ...
            <Card className="border-border/50">
              <CardContent className="p-4">
                <h3 className="font-bold text-foreground mb-3 text-sm uppercase tracking-wide flex items-center gap-2">
                  <Filter className="h-4 w-4" />{bn ? "ফিল্টার" : "Filters"}
                </h3>
                <FilterPanel />
              </CardContent>
            </Card>
            ...
            <Card className="border-border/50 bg-primary/5">
              <CardContent className="p-4 text-xs text-muted-foreground space-y-2">
                <div className="flex items-start gap-2">
                  ...
                  <span>{bn ? "শুধুমাত্র ভেরিফায়েড বিক্রেতাদের সাথে লেনদেন করুন।" : "Deal only with verified sellers."}</span>
                </div>
                <div className="flex items-start gap-2">
                  <Truck className="h-4 w-4 text-primary shrink-0 mt-0.5" />
                  <span>{bn ? "পণ্য পরীক্ষা করে তবেই অর্থ প্রদান করুন।" : "Inspect the item before paying."}</span>
                </div>
              </CardContent>
            </Card>
          </aside>
          ...
          <main className="flex-1 min-w-0">
            {isLoading ? (
              <div className={viewMode === "grid" ? "grid grid-cols-2 lg:grid-cols-4 gap-3" : "space-y-3"}>
              ...
              <>
                ...
                <Pagination page={currentPage} totalPages={totalPages} onChange={setPage} bn={bn} />
              </>
            ) : (
              <>
...
function Pagination({ page, totalPages, onChange, bn }: { page: number; totalPages: number; onChange: (p: number) => void; bn: boolean }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1 mt-6">
      ...
      {pages.map((p) => (
        <Button key={p} variant={p === page ? "default" : "outline"} size="sm" className="w-9" onClick={() => onChange(p)}>
          {p}
        </Button>
      ))}
      ...
      <Card className="border-border/50 hover:shadow-lg transition-all overflow-hidden h-full bg-background">
        <div className="relative">
          ...
          {listing.is_featured && (
            <Badge className="absolute top-2 left-2 bg-amber-500 text-white text-[10px] border-0">
              <Star className="h-3 w-3 mr-0.5 fill-white" />{bn ? "প্রমোটেড" : "Promoted"}
            </Badge>
          )}
          <button
        ...
        <CardContent className="p-3">
          <p className="text-base font-bold text-primary">
            ৳{listing.price > 0 ? listing.price.toLocaleString("bn-BD") : (bn ? "আলোচনা সাপেক্ষ" : "Negotiable")}
          </p>
          ...
          <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
            ...
            <span className="truncate">{listing.location_area || listing.location_district || listing.location_division}</span>
          </div>
...
function ListingListItem({ listing, onClick, bn }: { listing: DealListing; onClick: () => void; bn: boolean }) {
  const img = listing.images?.[0];
  return (
    <Card className="border-border/50 hover:shadow-md transition-shadow cursor-pointer bg-background" onClick={onClick}>
      <CardContent className="p-3 flex gap-3">
        ...
        {listing.is_featured && (
          <Badge className="bg-amber-500 text-white text-[10px] self-start shrink-0 border-0">
