...
import { Heart, ChevronRight, Eye, Clock, Tag, Star, Plus, MessageCircle, MapPin, Package, LayoutGrid } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { useLanguage } from "@/contexts/LanguageContext";
import { useDealCategoryTree, useFeaturedDeals, useLatestDeals, DealListing, DealCategory } from "@/hooks/useDealData";
...
const DealCard = React.forwardRef<HTMLDivElement, { listing: DealListing; onClick: () => void; bn?: boolean }>(({ listing, onClick, bn = true }, ref) => {
  const img = listing.images?.[0];
  return (
    <motion.div ref={ref} whileHover={{ y: -2 }} className="cursor-pointer" onClick={onClick}>
      <Card className="border-border/50 hover:shadow-lg transition-all overflow-hidden h-full">
          ...
          {listing.is_featured && (
            <div className="absolute top-0 left-0 z-10">
              <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[9px] font-bold px-3 py-0.5 rounded-br-lg rounded-tl-lg flex items-center gap-0.5 shadow-sm">
                <Star className="h-2.5 w-2.5 fill-white" /> {bn ? "প্রমোটেড" : "PROMOTED"}
              </div>
            </div>
          )}
        ...
        <CardContent className="p-3">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
            ...
            <span>{listing.location_area || listing.location_district || listing.location_division}</span>
          </div>
...
const DealHome = () => {
  ...
  const [locationFilter, setLocationFilter] = useState({ division: "", district: "", thana: "" });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero Search - bikroy style (flush, no grey gap) */}
      <div className="bg-gradient-to-b from-primary/10 to-background pt-[44px] pb-5 md:pt-[68px] md:pb-10">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-muted-foreground mb-6 text-sm">
            {bn ? "বাংলাদেশের সবচেয়ে বিশ্বস্ত কেনাবেচার প্ল্যাটফর্ম" : "Bangladesh's Most Trusted Buy & Sell Platform"}
          </p>
          ...
          {/* Search bar */}
          <DealSearchBox location={locationFilter.division || "all"} className="max-w-xl mx-auto" />

          {/* All Ads CTA - bikroy style */}
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 pb-28 md:pb-10">
        {/* Categories Grid - Bikroy style with subcategories */}
        <h2 className="text-lg font-bold text-foreground mb-4">
          {bn ? "ক্যাটাগরি অনুযায়ী ব্রাউজ করুন" : "Browse by Category"}
        </h2>
        {catLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-8">
            {Array(14).fill(0).map((_, i) => <Skeleton key={i} className="h-20 rounded-xl" />)}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
            {categoryTree?.map(cat => (
              ...
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {cat.children && cat.children.length > 0 && (
                      <span className="text-[10px] text-muted-foreground">
                        {cat.children.length} {bn ? "টি সাব-ক্যাটাগরি" : "subcategories"}
                      </span>
                    )}
                  </div>
                  {cat.children && cat.children.length > 0 && (
                ...
                <AnimatePresence>
                  {hoveredCat === cat.id && cat.children && cat.children.length > 0 && (
                    <motion.div
                    ...
                    >
                      ...
                      >
                        {bn ? `সকল ${cat.name} দেখুন` : `View all ${cat.name_en || cat.name}`}
        ...
        {/* Featured Ads - Premium Section */}
        {(featured?.length || 0) > 0 && (
          <div className="mb-10">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                <span className="bg-amber-500/10 p-1.5 rounded-lg"><Star className="h-5 w-5 text-amber-500 fill-amber-500" /></span>
                {bn ? "ফিচার্ড বিজ্ঞাপন" : "Featured Ads"}
              </h2>
              <Button variant="ghost" size="sm" onClick={() => navigate("/deal/ads?featured=1")} className="text-primary">
                {bn ? "সবগুলো দেখুন" : "View All"} <ChevronRight className="h-4 w-4" />
              </Button>
            </div>
              ...
              {featured?.map(listing => (
                <div key={listing.id} className="relative">
                  ...
                  <div className="relative z-10">
                    <DealCard listing={listing} onClick={() => navigate(`/deal/ad/${listing.id}`)} bn={bn} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="ghost" size="sm" onClick={() => navigate("/deal/ads")} className="text-primary">
            {bn ? "সবগুলো দেখুন" : "View All"} <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
        {latestLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {latest?.map(listing => (
              <DealCard key={listing.id} listing={listing} onClick={() => navigate(`/deal/ad/${listing.id}`)} bn={bn} />
            ))}
          </div>
        )}
        ...
        <div className="mt-8 text-center flex gap-3 justify-center flex-wrap">
          <Button size="lg" onClick={() => navigate("/deal/post")} className="rounded-xl text-base font-bold gap-2 px-8">
            <Plus className="h-5 w-5" /> {bn ? "ফ্রি বিজ্ঞাপন দিন" : "Post Free Ad"}
          </Button>
          <Button size="lg" variant="outline" onClick={() => navigate("/deal/my-ads")} className="rounded-xl text-base font-bold gap-2 px-8">
            <Package className="h-5 w-5" /> {bn ? "আমার বিজ্ঞাপন" : "My Ads"}
          </Button>
          <Button size="lg" variant="outline" onClick={() => navigate("/deal/inbox")} className="rounded-xl text-base font-bold gap-2 px-8">
            <MessageCircle className="h-5 w-5" /> {bn ? "ইনবক্স" : "Inbox"}
          </Button>
        </div>
      </div>
