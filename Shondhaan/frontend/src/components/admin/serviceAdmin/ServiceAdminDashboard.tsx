...
const apiFetch = async (url: string) => {
  ...
  if (!contentType || !contentType.includes("application/json")) {
    console.warn(`⚠️ Server returned non-JSON response for ${url}`);
    return null; 
  }
...
const ServiceAdminDashboard = () => {
  ...
  const load = async () => {
    setLoading(true);
    console.log(`🚀 Loading Service Dashboard data...`);
    
    try {
      // Fetch Service Bookings
      // Using relative URL '/api/bookings' is recommended. 
      // If you must use a full URL, ensure CORS is enabled on your backend.
      const bookingsRes = await apiFetch(`/api/bookings?status=pending&count=true`);
      const pendingBookings = bookingsRes?.count ?? 0;
      console.log(`📋 Pending Service Bookings: ${pendingBookings}`);

      // If you have a specific stats endpoint for service snapshots, put it here.
      // Example: const snapsData = await apiFetch(`/api/bookings/stats?limit=7`);
      ...
      const list: Alert[] = [];
      if (pendingBookings > 0) list.push({ type: "booking", label: "অপেক্ষমাণ সার্ভিস বুকিং", count: pendingBookings, href: "/admin/bookings", tone: "sky" });
      
      setAlerts(list);
      console.log("🔔 Final Service Alerts:", list);
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      {/* === HERO HEADER === */}
      <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-sky-500/8 via-card to-emerald-500/5 p-5 md:p-6">
        <div className="absolute -top-20 -right-20 h-56 w-56 rounded-full bg-sky-500/15 blur-3xl" />
        <div className="absolute -bottom-16 -left-16 h-48 w-48 rounded-full bg-emerald-500/10 blur-3xl" />
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="h-11 w-11 shrink-0 rounded-2xl bg-gradient-to-br from-sky-500 to-blue-600 text-white flex items-center justify-center shadow-lg shadow-sky-500/25">
              <Wrench className="h-5 w-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg md:text-2xl font-bold tracking-tight text-foreground">সার্ভিস ড্যাশবোর্ড</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Activity className="h-3.5 w-3.5" /> সার্ভিস বুকিং <ArrowUpRight className="h-3.5 w-3.5" />
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border/60 bg-card p-4 md:p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-foreground">সার্ভিস - গত ৭ দিনের পারফরম্যান্স</h3>
              <p className="text-[11px] text-muted-foreground mt-0.5">নতুন বুকিং vs রেভিনিউ</p>
            </div>
            <div className="flex items-center gap-3 text-[11px]">
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-sky-500" /> বুকিং</span>
            ...
            {chartData.length > 1 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 5, right: 10, bottom: 0, left: -20 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="bookings" stroke="rgb(14, 165, 233)" strokeWidth={2} fill="url(#g-bookings)" />
      ...
      <div>
        <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
          <Activity className="h-4 w-4 text-sky-500" /> দ্রুত একশন
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { to: "/admin/bookings", icon: Calendar, label: "সার্ভিস বুকিং", desc: "বুকিং দেখুন", tint: "text-sky-600 bg-sky-500/10" },
            { to: "/admin/requests", icon: Inbox, label: "সার্ভিস রিকোয়েস্ট", desc: "রিকোয়েস্ট ম্যানেজ", tint: "text-rose-600 bg-rose-500/10" },
            { to: "/admin/services", icon: Wrench, label: "সেবা CMS", desc: "সেবা আপডেট", tint: "text-emerald-600 bg-emerald-500/10" },
            { to: "/admin/service-images", icon: Activity, label: "সেবার ছবি", desc: "ইমেজ ম্যানেজ", tint: "text-violet-600 bg-violet-500/10" },
          ].map((q) => (
            <Link
              key={q.to}
              to={q.to}
              className="group rounded-2xl border border-border/60 bg-card p-3.5 hover:border-sky-500/40 hover:shadow-md transition-all"
            >
