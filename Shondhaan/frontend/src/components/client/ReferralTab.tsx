// src/components/client/ReferralTab.tsx

import { useState, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  Gift, Copy, Check, Users, Clock, Award, Wallet, 
  ArrowUpRight, Share2, QrCode, ExternalLink, RefreshCw
} from "lucide-react";
...
const ReferralTab = ({ onNavigateToPayments }: ReferralTabProps) => {
  ...
  const handleClaim = useCallback(async (rewardId: number) => {
    ...
    if (success && onNavigateToPayments) {
      // Optionally navigate to payments after claim
    }
  }, [claimReward, onNavigateToPayments]);
  ...
  const handleShare = useCallback(async () => {
    ...
    const shareData = {
      ...
      text: bn 
        ? `আমার রেফারেল লিংক দিয়ে সাইন আপ করুন এবং ৳২০ পুরস্কার পান!` 
        : `Sign up with my referral link and get ৳20 reward!`,
      url: stats.code.link,
    };
  ...
  }, [stats?.code?.link, bn, copyToClipboard]);
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-green-500 via-blue-500 to-rose-500 p-6 md:p-8 text-white"
      >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
            {[
              ...
              { 
                ...
                value: `৳${stats?.summary?.total_earned || 0}`, 
      ...
      {stats?.summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { 
              label: bn ? "মোট রেফার" : "Total Referrals", 
              value: stats.summary.total_referred, 
              icon: <Users className="h-4 w-4" />,
              color: "text-blue-600", 
              bg: "bg-blue-50" 
            },
            { 
              label: bn ? "পেন্ডিং" : "Pending", 
            ...
            { 
              label: bn ? "পুরস্কার প্রাপ্ত" : "Rewarded", 
              value: stats.summary.rewarded, 
