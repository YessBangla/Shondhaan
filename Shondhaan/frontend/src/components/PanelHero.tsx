...
const PanelHero = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br text-primary-foreground p-5 md:p-6 shadow-lg",
        gradient,
        className
      )}
    >
      ...
      {stats && stats.length > 0 && (
        <div className="relative mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          {stats.map((s, i) => (
            ...
            >
              <div className="flex items-center gap-1.5 text-[10px] font-semibold text-primary-foreground/80 uppercase tracking-wide">
                {s.icon}
                {s.label}
              </div>
